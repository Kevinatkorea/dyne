...
**보고서 사진** · 사업영역 · 보유장비 · 연혁 · 고객사 · 수상인증 · 상단배경 · 공지팝업 · 미디어 ·
사이트설정 · 관리자계정 · 백업복원 · 감사로그

## 메인 [보고서 전문 서비스] 섹션

주사업이 보고서라 SERVICES 그리드 위에 별도 섹션(`ReportSection`)으로 먼저 보여 준다.

- 사진은 관리자 **[보고서 사진]**(`reports` 테이블)에서 관리한다. 등록 개수가 곧 열 개수 —
  5건이면 5열, 6건이면 6열. 좁은 화면에서는 `styles.css` 의 `.report-shots` 가
  3열(≤1024) · 2열(≤768)로 다시 묶는다.
- 등록분이 없으면 `home.jsx` 의 `REPORT_FALLBACK` 4장이 그대로 나온다(서버 없이도 동작).
- 연도 아카이브 사진에는 원본 왼쪽 아래에 발주처 캡션이 박혀 있다. 그대로 쓰면 글자가
  잘린 채 노출되므로, 잘라낸 사본을 `resource/report/` 에 두고 그쪽을 쓴다.
- 푸터 우측 네이버 블로그 아이콘 주소는 설정 `footer.blogUrl` (비우면 아이콘이 사라진다).
